	...
	while (i < rows)
	{
		...
		while (j < cols)
		{
			if (i == 0 || i == rows - 1 || j == 0 || j == cols - 1)
			{
				if (map[i][j] != '1')
				{
					printf("Error: need a wall at (%d, %d)\n", i, j);
					return (0);
				}
			}
			j++;
...
int	error_handling(t_vars *vars)
{
	if (vars->player != 1)
	{
		printf("Error: Map must contain one player. Found: %d\n", vars->player);
		return (0);
	}
	if (vars->exit != 1)
	{
		printf("Error: Map must contain one exit. Found: %d\n", vars->exit);
		return (0);
	}
	if (vars->coin < 1)
	{
		printf("Error: Map must contain >= 1 coin. Found: %d\n", vars->coin);
		return (0);
	}
	return (1);
}
